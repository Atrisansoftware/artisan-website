...
//   2. Provider fallback order (Groq -> DeepSeek -> OpenRouter -> Nvidia ->
//      Gemini) stays fixed and tested, because reliability from a
//      known-working chain beats a fragile "pick the best provider"
//      heuristic with no real data behind it.
...
module.exports = async function handler(req, res) {
  ...
  const hasAnyProvider =
    process.env.GROQ_API_KEY || process.env.DEEPSEEK_API_KEY || process.env.OPENROUTER_API_KEY ||
    process.env.NVIDIA_API_KEY || process.env.GEMINI_API_KEY;
  if (!hasAnyProvider) {
    console.error('No AI provider API key is set (need at least one of GROQ_API_KEY, DEEPSEEK_API_KEY, OPENROUTER_API_KEY, NVIDIA_API_KEY, GEMINI_API_KEY).');
