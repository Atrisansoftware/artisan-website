...
function setupMailtoForm(formEl) {
  ...
  formEl.addEventListener('submit', function (e) {
    ...
    showStatus('loading', 'Preparing your message...');
    ...
    if (fileInput && fileInput.files && fileInput.files.length) {
      lines.push('Attached file (please attach manually): ' + Array.from(fileInput.files).map(f => f.name).join(', '));
    }
    const subject = formEl.dataset.subject || 'Website Inquiry — Artisan Engineering and Builders';
    const body = lines.join('\n');
    const mailtoUrl = 'mailto:' + toEmail + '?subject=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(body);

    setTimeout(() => {
      window.location.href = mailtoUrl;
      showStatus('success', 'Your email app should now open with your message ready to send to ' + toEmail + '. Please hit send there to complete your submission.');
...
/* =========================================================
   Artisan AI workspace — chat, file upload, and functional tools.
   Honest architecture: genuinely computes what a static frontend
   can (conversions, quantity math, BOQ totals, report assembly,
   plain-text file reading) and clearly says so when something
   needs a connected AI backend that isn't configured here.
   ========================================================= */
(function () {
  ...
  // Real backend: routes to Groq / DeepSeek / OpenRouter / Nvidia / Gemini
  // (whichever has a key configured), with company knowledge, real tool-calling
  // for calculations, and project memory. Deployed as a Vercel serverless
  // function at /api/artisan-ai — see api/artisan-ai.js.
  ...
  // Honest fallback: if the company backend isn't reachable (e.g. this page is
  // being viewed as static files rather than deployed on Vercel, or every
  // configured provider is down), fall back to Puter.js — a real, free,
  // no-API-key AI service that runs entirely client-side. Still a genuine AI
  // response, just without Artisan's company knowledge and calculation tools.
    ...
    callBackend(val, historyForApi, projectContext)
      ...
      .catch(() => {
        callPuterFallback(val, historyForApi)
          ...
          .catch(() => {
            const msg = "I couldn't reach Artisan AI's servers or the backup AI service just now. Please try again in a moment, use the Smart Tools below (Unit Converter, BOQ Analyzer, Calculations — these work with no connection at all), or reach our team directly via Talk to Advisor.";
  ...
  document.querySelectorAll('[data-chat-prompt]').forEach(btn => {
    btn.addEventListener('click', () => {
      chatInput.value = btn.dataset.chatPrompt;
      chatInput.scrollIntoView({ behavior: 'smooth', block: 'center' });
      chatInput.focus();
    });
  });
  function clearChat() {
    chatWindow.innerHTML = '<div class="chat-msg ai">Hello! I\'m Artisan AI, your engineering assistant. Upload files, ask questions, or try one of the tools below and I\'ll help however I honestly can.</div>';
  }
  const heroNewChat = document.getElementById('hero-new-chat');
  const sidebarNewChat = document.getElementById('sidebar-new-chat');
  const finalNewChat = document.getElementById('final-new-chat');
  [heroNewChat, sidebarNewChat, finalNewChat].forEach(b => { if (b) b.addEventListener('click', clearChat); });

  // Sidebar panel buttons (History/Files/Drafts/Saved) -> honest inline note in chat
    ...
    btn.addEventListener('click', () => {
      ...
      if (panel === 'history') {
        ...
        if (chatHistory.length === 0) {
          note.textContent = 'No previous chats yet in this session.';
        } else {
          note.textContent = 'This session has ' + chatHistory.length + ' previous message(s). Most recent: "' + chatHistory[chatHistory.length - 1].user + '"';
        }
        ...
        const note = document.createElement('div');
        note.className = 'chat-msg system-note';
        note.textContent = panel === 'drafts' ? 'Drafts are saved locally in your session as you work — none yet.' : 'Saved results will appear here once you save an output from a tool.';
        chatWindow.appendChild(note);
      }
    });
  });

  /* ---------- File upload: real client-side reading where possible ---------- */
  ...
  function addRecentFile(file, extraNote) {
    if (uploadedCount === 0) recentList.innerHTML = '';
    uploadedCount++;
    const row = document.createElement('div');
    row.className = 'rf-item';
    row.innerHTML = '<div class="rf-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19V5a2 2 0 012-2h9l5 5v11a2 2 0 01-2 2H6a2 2 0 01-2-2z"/></svg></div><div><strong>' + file.name + '</strong><span>' + humanSize(file.size) + (extraNote ? ' &middot; ' + extraNote : '') + '</span></div>';
    recentList.appendChild(row);
  }

  function handleFiles(files) {
    Array.from(files).forEach(file => {
      addRecentFile(file, null);
      const note = document.createElement('div');
      ...
      if (/text\/plain/.test(file.type) || /\.txt$/i.test(file.name)) {
        ...
        note.textContent = 'Received "' + file.name + '" (' + humanSize(file.size) + ', ' + (file.type || 'unknown type') + '). Full content analysis for this file type needs a connected AI backend that isn\'t configured in this environment yet — but the file details above are real.';
      }
      chatWindow.appendChild(note);
      chatWindow.scrollTop = chatWindow.scrollHeight;
    });
  }
  setupDropzone('ai-dropzone', 'ai-file-input', null);
  const aiFileInput = document.getElementById('ai-file-input');
  if (aiFileInput) aiFileInput.addEventListener('change', () => handleFiles(aiFileInput.files));
  const aiDropzone = document.getElementById('ai-dropzone');
  if (aiDropzone) aiDropzone.addEventListener('drop', (e) => { if (e.dataTransfer.files.length) handleFiles(e.dataTransfer.files); });
  ...
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && toolModal.classList.contains('open')) closeTool(); });
  ...
  const TOOLS = {
    converter() {
      ...
      `, () => {
        ...
        [cat].forEach(el => el.addEventListener('change', fillUnits));
      ...
      `, () => {
        ...
        function compute() {
          ...
          if (typeSel.value === 'area') {
            ...
            const bricksPerM3 = 500; // typical approx. for standard brick + mortar, verify locally
    ...
    report() {
      openTool('Report Generator', `
        <div class="form-grid">
          <div class="form-field full"><label>Report Type</label><select id="rp-type"><option>Site Progress Report</option><option>Project Summary Report</option><option>Inspection Report</option></select></div>
          <div class="form-field full"><label>Project Name</label><input type="text" id="rp-name" placeholder="e.g. Modern Residential House, Nepalgunj"></div>
          <div class="form-field full"><label>Scope / Summary</label><textarea id="rp-scope" placeholder="Brief description of the work covered"></textarea></div>
          <div class="form-field full"><label>Key Points (one per line)</label><textarea id="rp-points" placeholder="Foundation work completed&#10;Structural framing 60% complete&#10;Next: roofing"></textarea></div>
        </div>
        <button type="button" class="btn btn-primary btn-sm" id="rp-generate" style="margin-top:12px;">Generate <span>&rarr;</span></button>
        <div class="tool-result" id="rp-result">
          <div class="lbl">Preview</div>
          <pre id="rp-preview" style="white-space:pre-wrap; font-family:inherit; font-size:12.5px; color:var(--ink); margin:8px 0;"></pre>
          <button type="button" class="btn btn-outline btn-sm" id="rp-download">Download Report (.txt)</button>
        </div>
      `, () => {
        document.getElementById('rp-generate').addEventListener('click', () => {
          const type = document.getElementById('rp-type').value;
          const name = document.getElementById('rp-name').value || 'Untitled Project';
          const scope = document.getElementById('rp-scope').value || '\u2014';
          const points = (document.getElementById('rp-points').value || '').split('\n').filter(Boolean);
          const date = new Date().toLocaleDateString();
          let text = type.toUpperCase() + '\n' + '='.repeat(type.length) + '\n\n';
          text += 'Project: ' + name + '\nDate: ' + date + '\nPrepared by: Artisan Engineering and Builders Pvt. Ltd.\n\n';
          text += 'Scope / Summary:\n' + scope + '\n\n';
          text += 'Key Points:\n' + (points.length ? points.map(p => '- ' + p).join('\n') : '- \u2014') + '\n';
          document.getElementById('rp-result').classList.add('show');
          document.getElementById('rp-preview').textContent = text;
          document.getElementById('rp-download').onclick = () => {
    ...
    summarizer() {
      openTool('Document Summarizer', `
        <p class="desc" style="margin-bottom:10px;">Upload a plain text (.txt) file for a real, in-browser summary. Other formats (PDF/DOCX) need a connected backend to extract text, which isn't configured in this environment yet.</p>
        <input type="file" id="sm-file" accept=".txt">
        <div class="tool-result" id="sm-result"><div class="lbl">Summary</div><p id="sm-text" style="font-size:13px; color:var(--ink); margin:6px 0 0;"></p></div>
      `, () => {
        document.getElementById('sm-file').addEventListener('change', (e) => {
          const file = e.target.files[0];
          if (!file) return;
          if (!/\.txt$/i.test(file.name)) {
            document.getElementById('sm-result').classList.add('show');
            document.getElementById('sm-text').textContent = 'This file type needs a connected backend for text extraction. Please upload a .txt file for a real summary right now.';
            return;
          }
          const reader = new FileReader();
          reader.onload = () => {
            ...
            const summary = sentences.slice(0, 3).join(' ') || '(File appears to be empty.)';
            const words = content.trim().split(/\s+/).filter(Boolean).length;
            document.getElementById('sm-result').classList.add('show');
            document.getElementById('sm-text').textContent = 'Document length: ' + words + ' words. Basic extractive summary (first 3 sentences): ' + summary;
          };
          reader.readAsText(file);
        });
      });
    ...
    'code-helper'() {
      openTool('Code & Standard Helper', `
        <p class="desc">To avoid giving you inaccurate or fabricated code clauses, Artisan AI does not generate specific code text without a connected, verified standards backend (not configured in this environment).</p>
        <p class="desc">For Nepal-based structural and building work, commonly referenced standards include the National Building Code of Nepal (NBC) series and relevant IS codes. Always confirm exact clauses against the official published standard or with your engineer — please <a href="contact.html">contact our team</a> for project-specific guidance.</p>
      `);
    },
    'design-checker'() {
      openTool('Design Checker', `
        <p class="desc" style="margin-bottom:10px;">Upload a design file or image below. Artisan AI will confirm real file details; a full automated design review needs a connected AI backend that isn't configured in this environment yet.</p>
        <input type="file" id="dc-file" accept="image/*,.pdf,.dwg">
        <div class="tool-result" id="dc-result"><div class="lbl">File Details</div><p id="dc-text" style="font-size:13px; color:var(--ink); margin:6px 0 0;"></p></div>
      `, () => {
        document.getElementById('dc-file').addEventListener('change', (e) => {
          const file = e.target.files[0];
          if (!file) return;
          document.getElementById('dc-result').classList.add('show');
          document.getElementById('dc-text').textContent = 'Received "' + file.name + '" (' + humanSize(file.size) + '). For a genuine automated design review (structural checks, code compliance, clash detection) we would need to connect a real backend engine — please have this reviewed by our engineering team via Contact Us for now.';
        });
      });
    },
    cad() {
      openTool('CAD & Plan Analyzer', `
        <p class="desc" style="margin-bottom:10px;">Upload a CAD file (DWG/DXF/PDF) to see its real file details. Parsing actual drawing geometry needs a connected backend CAD engine, which isn't configured in this environment yet.</p>
        <input type="file" id="cad-file" accept=".dwg,.dxf,.pdf">
        <div class="tool-result" id="cad-result"><div class="lbl">File Details</div><p id="cad-text" style="font-size:13px; color:var(--ink); margin:6px 0 0;"></p></div>
      `, () => {
        document.getElementById('cad-file').addEventListener('change', (e) => {
          const file = e.target.files[0];
          if (!file) return;
          document.getElementById('cad-result').classList.add('show');
          document.getElementById('cad-text').textContent = 'Received "' + file.name + '" (' + humanSize(file.size) + '). Real geometry/measurement extraction needs a connected CAD-parsing backend — not yet configured here. Our team can review the actual drawing directly; see Contact Us.';
        });
      });
    },
    'image-gen'() {
      openTool('Image & Design Generation', `
        <div class="form-field"><label>Describe the image you want</label><textarea id="ig-prompt" placeholder="e.g. Modern two-storey house with glass facade and flat roof"></textarea></div>
        <button type="button" class="btn btn-primary btn-sm" id="ig-generate" style="margin-top:10px;">Generate <span>&rarr;</span></button>
        <div class="tool-result" id="ig-result"><p id="ig-text" style="font-size:13px; color:var(--ink); margin:0;"></p></div>
      `, () => {
        document.getElementById('ig-generate').addEventListener('click', () => {
          const prompt = document.getElementById('ig-prompt').value.trim();
          document.getElementById('ig-result').classList.add('show');
          document.getElementById('ig-text').textContent = prompt
            ? 'Your prompt is ready: "' + prompt + '". Image generation requires a connected image-generation API, which isn\'t configured in this environment yet — once connected, this exact workflow will send your prompt and display the generated image here.'
            : 'Please enter a description first.';
        });
      });
    },
    upload() {
      openTool('Upload & Analyze', `<p class="desc">Use the Upload &amp; Analyze panel in the main Artisan AI workspace above to add files — this closes so you can drop files there.</p>`);
      setTimeout(() => {
        closeTool();
        document.getElementById('ai-dropzone').scrollIntoView({ behavior: 'smooth', block: 'center' });
      }, 900);
